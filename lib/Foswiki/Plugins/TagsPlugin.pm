...
use vars qw( $VERSION $RELEASE $SHORTDESCRIPTION $debug $pluginName $NO_PREFS_IN_TOPIC $doneLoadTemplate );
$VERSION = '$Rev$';
...
sub afterSaveHandler {
    ...
    updateTopicTags('topic', $_[2], $_[1], getUserId($Foswiki::Plugins::SESSION));
...
sub _TAGLIST {
    ...
    my $header = $params->{header} || Foswiki::Func::expandTemplate('tagsplugin:'.$displayType.':header') || '';
    my $format = $params->{format} || Foswiki::Func::expandTemplate('tagsplugin:'.$displayType.':format') || '$tag';
    my $sep = $params->{separator} || Foswiki::Func::expandTemplate('tagsplugin:'.$displayType.':separator') || ', ';
    my $footer = $params->{footer} || Foswiki::Func::expandTemplate('tagsplugin:'.$displayType.':footer') || '';
    ...
    if (defined($filter) && $filter ne '') {
       push @joinsClauses, " INNER JOIN UserItemTag fi2t ON fi2t.item_id = i2t.tag_id ";
...
my $retreiveNameFrom = 't';
    ...
    my $db = new Foswiki::Contrib::DbiContrib;
    my $statement = "SELECT $retreiveNameFrom.item_name, ts.num_items FROM UserItemTag i2t
...
#print STDERR "$statement";
    ...
    foreach my $row (@{$arrayRef}) {
        ...
        $escapedTag =~ s/\s/\+/g;
        my $tagurl = '%SCRIPTURL{view}%/%SYSTEMWEB%/'.$pluginName.'Views?tag='.$escapedTag;
        my $tag_count = $row->[1];

        my $entry = $format;
        $entry =~ s/\$tagrange/not-popular/g;  #TODO: from (not-popular not-very-popular somewhat-popular popular very-popular ultra-popular)
...
sub tagCall {
   ...
   if (defined($redirectto)) {
       ...
       my ($web, $topic) = Foswiki::Func::normalizeWebTopicName('', $item_name);
...
#    $FoswikiCuid = '666' if ($FoswikiCuid =~ /BaseUserMapping_/);
    ...
    my $db = new Foswiki::Contrib::DbiContrib;
    my $cuid;
    my $statement = sprintf('SELECT %s from %s WHERE %s = ? ',  qw( CUID Users FoswikicUID));
    my $arrayRef = $db->dbSelect($statement, $FoswikiCuid);
    if (defined($arrayRef->[0][0])) {
	    $cuid = $arrayRef->[0][0];
    } else {
        $statement = sprintf('INSERT INTO %s (%s) VALUES (?)',  qw(Users FoswikicUID));
...
sub tagItem {
    ...
    my $db = new Foswiki::Contrib::DbiContrib;
    my $item_id;
    my $statement = sprintf('SELECT %s from %s WHERE %s = ? AND %s = ?',  qw( item_id Items item_name item_type));
    my $arrayRef = $db->dbSelect($statement, $item_name, $item_type);
    if (defined($arrayRef->[0][0])) {
	    $item_id = $arrayRef->[0][0];
    } else {
        $statement = sprintf('INSERT INTO %s (%s, %s) VALUES (?,?)',  qw( Items item_name item_type));
        my $rowCount = $db->dbInsert($statement, $item_name, $item_type);
        $statement = sprintf('SELECT %s from %s WHERE %s = ? AND %s = ?',  qw( item_id Items item_name item_type));
	...
	$tag_id = $arrayRef->[0][0];
        ...
        $statement = sprintf('INSERT INTO %s (%s) VALUES (?)',  qw( TagStat tag_id));
        $db->dbInsert($statement, $tag_id);
        $statement = sprintf('INSERT INTO %s (%s,%s) VALUES (?,?)',  qw( UserTagStat tag_id user_id));
        $db->dbInsert($statement, $tag_id, $user_id);
        $new_tag = 1;
    }

    my $rowCount = 0;
    $statement = sprintf('SELECT %s from %s WHERE %s = ? AND %s = ? AND %s = ?',  qw(tag_id UserItemTag user_id item_id tag_id));
    $arrayRef = $db->dbSelect($statement, $user_id, $item_id, $tag_id);
    if (!defined($arrayRef->[0][0])) {
        $statement = sprintf('INSERT INTO %s (%s, %s, %s) VALUES (?,?,?)',  qw( UserItemTag user_id item_id tag_id));
        $rowCount = $db->dbInsert($statement, $user_id, $item_id, $tag_id);

        unless ($new_tag) {
            $statement = sprintf('UPDATE %s SET %s=%s+1 WHERE %s = ?',  qw( TagStat num_items num_items tag_id));
            my $modified = $db->dbInsert($statement, $tag_id);
            if ($modified == 0) {
                $statement = sprintf('INSERT INTO %s (%s) VALUES (?)',  qw( TagStat tag_id));
                $db->dbInsert($statement, $tag_id);
            }
            $statement = sprintf('UPDATE %s SET %s=%s+1 WHERE %s = ? AND %s = ?',  qw( UserTagStat num_items num_items tag_id user_id));
