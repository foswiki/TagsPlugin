...
sub do {
    ...
    # if public=0, set (new) user_id, too.
    #
    my $affected_rows = 0;
    my $current_user_id = Foswiki::Plugins::TagsPlugin::getUserId( Foswiki::Func::getCanonicalUserID() );
    if ($public eq "0") {
      $statement = sprintf( 'UPDATE %s SET public=0, %s = ? WHERE %s = ? AND %s = ? AND %s = ? AND public=1',
        qw( UserItemTag user_id item_id tag_id user_id ) );
      $affected_rows = $db->dbInsert( $statement, $current_user_id, $item_id, $tag_id, $user_id );
    } else {
      $statement = sprintf( 'UPDATE %s SET public=1 WHERE %s = ? AND %s = ? AND %s = ? AND public=0',
        qw( UserItemTag item_id tag_id user_id ) );
      $affected_rows = $db->dbInsert( $statement, $item_id, $tag_id, $user_id );
    }
    Foswiki::Func::writeDebug("TagsPlugin::Public: $statement, pub:$public, item:$item_id, tag:$tag_id, user:$user_id cur:$current_user_id" ) if DEBUG;
